...
const generateToken = (userId, expires, type, secret = config.jwt.secret) => {
  var token = jwt.sign({"sub": userId , "type": type , exp : expires}, secret);
...
const generateAuthTokens = async (user) => {
  var expiryTime = new Date(new Date().getTime() + config.jwt.accessExpirationMinutes*60000);
  var token = generateToken(user._id , Math.round(expiryTime.getTime()/1000), tokenTypes.ACCESS );
