...
const getUser = catchAsync(async (req, res) => {
    ...
    if( userId != req.user._id) throw new ApiError(httpStatus.FORBIDDEN, "User not found");
    //console.log(req.query.length);
    if( req.query.q ){
      //console.log("ADDRESS QUERY")
      await userService.getUserAddressById(userId).then((value) => {
        if( value == null ){
          throw new ApiError(httpStatus.NOT_FOUND, "User not found");
        }
        else res.status(200).send({address: value.address} );
      })
